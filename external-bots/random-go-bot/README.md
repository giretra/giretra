# Random Go Bot — Giretra Bot Template

A starter template for building a Giretra bot in Go. This bot plays randomly — use it as a foundation and replace the logic with your own strategy.

## Quick Start

```bash
# Build
go build -o random-go-bot .

# Run (default port 5063)
./random-go-bot

# Run on a custom port
PORT=8080 ./random-go-bot
```
...
| File | Purpose |
|------|---------|
| **`bot.go`** | Your game logic. **This is the only file you need to edit.** |
| `types.go` | Type definitions for the API (cards, game state, contexts). Read-only reference. |
| `server.go` | HTTP server boilerplate. Routes requests to your `Bot` methods. Do not edit. |
...
Edit `bot.go`. You have three decision methods and five notification hooks:
...
```go
func (b *Bot) OnDealStarted(ctx DealStartedContext)      // New deal begins
func (b *Bot) OnCardPlayed(ctx CardPlayedContext)         // Any player played a card
func (b *Bot) OnTrickCompleted(ctx TrickCompletedContext) // Trick finished, with winner
func (b *Bot) OnDealEnded(ctx DealEndedContext)           // Deal scored
func (b *Bot) OnMatchEnded(ctx MatchEndedContext)         // Match over
```
...
- **One Bot instance per match** — you can store state across deals in struct fields.
