# Random Java Bot — Giretra Bot Template

A starter template for building a Giretra bot in Java 17. This bot plays randomly — use it as a foundation and replace the logic with your own strategy.
...
# Run (default port 5063)
java -jar target/random-java-bot.jar

# Run on a custom port
PORT=8080 java -jar target/random-java-bot.jar
```
...
| File | Purpose |
|------|---------|
| **`Bot.java`** | Your game logic. **This is the only file you need to edit.** |
| `BotTypes.java` | Type definitions for the API (cards, game state, contexts). Read-only reference. |
| `Server.java` | HTTP server boilerplate. Routes requests to your `Bot` methods. Do not edit. |
...
```
Game Server ──HTTP──► Your Bot (http://localhost:5063)
```
...
Edit `Bot.java`. You have three decision methods and five notification hooks:

### Decision Methods (must return a value)

```java
// Cut the deck (position 6–26, from top or bottom)
CutResult chooseCut(ChooseCutContext ctx)

// Pick a negotiation action from ctx.validActions()
NegotiationActionChoice chooseNegotiationAction(ChooseNegotiationActionContext ctx)

// Pick a card to play from ctx.validPlays()
Card chooseCard(ChooseCardContext ctx)
```

### Notification Hooks (optional, fire-and-forget)

```java
void onDealStarted(DealStartedContext ctx)          // New deal begins
void onCardPlayed(CardPlayedContext ctx)             // Any player played a card
void onTrickCompleted(TrickCompletedContext ctx)     // Trick finished, with winner
void onDealEnded(DealEndedContext ctx)               // Deal scored
void onMatchEnded(MatchEndedContext ctx)             // Match over
```
...
```json
{
  ...
  "launch": {
    "fileName": "java",
    "arguments": "-jar target/my-bot.jar",
    "startupTimeout": 15,
